...
public class ContactController extends GenericController {
    ...
    public ResponseEntity getAllContact() {
        ...
        try {
            return this.createStructuredResponse(this.contactsService.getAllContacts(), HttpStatus.OK);
        }
        catch (MyException e) {
            return this.forwardException("Impossible de récupérer les informations d'execution", e);
        }
    }
     ...
     */
    @RequestMapping(value = "/create", method = POST, produces = {MediaType.APPLICATION_JSON_VALUE})
    public ResponseEntity saveContact(@RequestBody ContactDTO contact) {
        LOGGER.info("Request the saving of contact {}", contact);
        try {
            return this.createStructuredResponse(this.contactsService.saveContact(contact), HttpStatus.OK);
        }
        catch (MyException e) {
